...
import { Clock, CheckCircle2, AlertCircle, MousePointer, Eye, ShoppingCart, CreditCard } from 'lucide-react';
  ...
  const { data: events, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tiktok_events')
        .select('id, event_type, event_id, value, currency, utm_campaign, ttclid, created_at')
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1 truncate">
                      ID: {event.event_id}
                      {event.ttclid && ` • ttclid: ${event.ttclid.slice(0, 20)}...`}
                    </p>
                  </div>
